...
$$\operatorname{tr}(X)=\sum_{a'}\langle a'|X|a'\rangle. \tag{1.5.14}$$

尽管在定义中使用了一组特定的基右矢，但可以证明$\operatorname{tr}(X)$是不依赖于表象的，如下所示：
...
$$\operatorname{tr}(XY)=\operatorname{tr}(YX), \tag{1.5.16a}$$
$$\operatorname{tr}(U^\dagger XU)=\operatorname{tr}(X), \tag{1.5.16b}$$
$$\operatorname{tr}(|a'\rangle\langle a''|)=\delta_{a'a''}, \tag{1.5.16c}$$
$$\operatorname{tr}(|b'\rangle\langle a'|)=\langle a'|b'\rangle. \tag{1.5.16d}$$
